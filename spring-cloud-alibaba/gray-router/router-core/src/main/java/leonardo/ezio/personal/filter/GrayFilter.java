...
public class GrayFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (GRAY_USER.contains(authorization)){
            ...
            String grayVersion = "2.0.0";
            log.info(String.format("request need router ! gray version : %s",grayVersion));
            GrayContextHolder.putContext(grayVersion);
            exchange = exchange.mutate().request(request.mutate().header(GrayConstant.GRAY_HEAD, grayVersion).build()).build();
        }
